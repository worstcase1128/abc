...
output_files = []
mode = 1
f_log = osp.join(result_dir, 'refactor_abc.txt')
result_sheet = osp.join(result_dir, "refactor_abc.csv")
f_log = open(f_log, 'w')
...
for case in benchmarks:
    for add_node in add_nodes:
        ...
        output_path = osp.join(output_dir, case + "_refactor_abc.aig")
        case_result = []
        print("===== [case] %s =====" % case)
        ...
        case_time = []
        case_stats = []
        abc_cmd = '%s -c "read %s; time; refactor ; time; print_stats; write test.aig"' % (abc_bin, case_path)
        print(abc_cmd)
